          ...
          >
            ...
            <div className="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col gap-3">
              <div className="text-center text-xs text-gray-400 mb-2">
                Welcome to Gov E-Services Support
              </div>
              {messages.map((msg, i) => (
                <div
                  key={i}
                ...
                >
                  <div
                    className={`max-w-[80%] p-3 rounded-2xl ${msg.senderType === "user" ? "bg-gov-blue text-white rounded-br-none" : "bg-white border border-gray-200 text-gray-800 rounded-bl-none"}`}
                  >
                    <p className="text-sm">{msg.content}</p>
                    <span
                      className={`text-[10px] mt-1 block ${msg.senderType === "user" ? "text-blue-200" : "text-gray-400"}`}
                    >
            ...
            >
              ...
              <button
                type="submit"
                className="bg-gov-blue text-white p-2 rounded-full hover:bg-blue-700 transition-colors"
                disabled={!input.trim()}
              >
                <Send size={18} />
              </button>
