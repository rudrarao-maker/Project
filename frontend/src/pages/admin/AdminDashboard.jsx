    ...
    <div className="premium-dashboard">
      ...
      >
        ...
        <div className="flex gap-3">
          <button
            onClick={exportToCSV}
            className="btn-outline-glow text-blue-500 border-blue-500"
          >
            <Download size={18} /> Export CSV
          </button>
          <button
            onClick={handleSetup2FA}
            className="btn-outline-glow text-green-500 border-green-500"
          >
      ...
      {show2FA && (
        <div
          className="fixed inset-0 bg-black/60 z-50 flex-center"
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleVerify2FA} className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={isVerifying || token2FA.length !== 6}
                className="btn-solid-blue px-6"
              >
                Verify
              </button>
            </form>
      ...
      >
        ...
        <motion.div variants={fadeInUp} className="stat-card">
          ...
          <div className="stat-content">
            <div className="stat-value">
              {overview.pendingApplications.toLocaleString()}
            </div>
      ...
      >
        <motion.div variants={fadeInUp} className="dashboard-panel mb-6">
          <div className="panel-header">
            <h2 className="panel-title">
              <Activity size={20} className="text-gov-blue" /> Growth Trends (6
              Months)
            </h2>
          </div>
        ...
        <motion.div variants={fadeInUp} className="dashboard-panel mb-6">
          <div className="panel-header">
            <h2 className="panel-title">
              <BarChart3 size={20} className="text-gov-blue" /> Application
              Status
            </h2>
          </div>
      ...
      >
        <motion.div variants={fadeInUp} className="dashboard-panel">
          <div className="panel-header">
            <h2 className="panel-title">
              <BarChart3 size={20} className="text-gov-blue" /> Top Services
            </h2>
          </div>
          <div className="panel-body" style={{ height: "350px" }}>
            <Bar
        ...
        <motion.div variants={fadeInUp} className="dashboard-panel">
          <div className="panel-header">
            <h2 className="panel-title">
              <Clock size={20} className="text-gov-blue" /> Recent Activity
            </h2>
          </div>
          <div
              ...
              <tbody>
                {recentActivity.map((act, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      {act.serviceName.length > 20
                        ? act.serviceName.substring(0, 20) + "..."
                        : act.serviceName}
                    </td>
                    <td>
                      <span
                        className={`status-badge ${act.status === "approved" ? "approved" : act.status === "rejected" ? "rejected" : "pending"}`}
                      >
