    ...
    <div className="premium-dashboard h-[calc(100vh-100px)] flex flex-col">
      <div className="dashboard-header mb-4 shrink-0">
        <h1 className="dashboard-title flex items-center gap-2">
          <MessageSquare className="text-gov-blue" /> Live Support Chat
        </h1>
        <p className="dashboard-subtitle">
          Monitor and respond to citizen inquiries
        </p>
      </div>
          ...
          {currentSession ? (
            <>
              <div className="p-4 border-b border-gray-200 bg-white flex items-center gap-3 shrink-0">
                <div className="bg-gov-blue p-2 rounded-full text-white">
                  <User size={18} />
                </div>
                <div>
              ...
              <div className="flex-1 p-6 overflow-y-auto bg-gray-50 flex flex-col gap-4">
                {messages.length === 0 ? (
                  ...
                  messages.map((msg, i) => (
                    ...
                    >
                      <div
                        className={`max-w-[70%] p-3 rounded-xl ${msg.senderType === "admin" ? "bg-gov-blue text-white rounded-br-none" : "bg-white border border-gray-200 text-gray-800 rounded-bl-none"}`}
                      >
                        <p className="text-sm">{msg.content}</p>
                        <span
                          className={`text-[10px] mt-1 block ${msg.senderType === "admin" ? "text-blue-200" : "text-gray-400"}`}
                        >
              ...
              <form
                ...
                className="p-4 bg-white border-t border-gray-200 flex gap-3 shrink-0"
              >
                <input
                  type="text"
                ...
                <button
                  type="submit"
                  disabled={!input.trim()}
                  className="btn-solid-blue px-6 py-2"
                >
                  <Send size={18} />
                </button>
              </form>
