...
import { Search, UserCheck, UserX, Clock, MoreVertical, ShieldAlert } from 'lucide-react';
    ...
    <div className="premium-dashboard">
      ...
      >
        <div>
          ...
          <p className="dashboard-subtitle">View, search, and manage registered citizens</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <motion.div variants={fadeInUp} className="dashboard-panel">
          <div className="panel-header">
            <h2 className="panel-title"><UserCheck size={20} className="text-gov-blue" /> Registered Users</h2>
          </div>
                ...
                <tbody>
                  {users.map(u => (
                    <tr key={u.id}>
                      ...
                      <td>{u.mobile}</td>
                      <td>
                        <span className={`status-badge ${u.status === 'active' ? 'approved' : u.status === 'suspended' ? 'rejected' : 'pending'}`}>
                          {u.status === 'active' && <UserCheck size={12} />}
                          {u.status === 'suspended' && <UserX size={12} />}
                          {u.status === 'pending_verification' && <Clock size={12} />}
                          {u.status.replace('_', ' ')}
                        </span>
                      </td>
