...
export default function AdminJobs() {
  ...
  const fetchJobs = async () => {
    try {
      const res = await adminService.getJobs({ search });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await adminService.updateJob(editing.id, form);
  ...
  const handleEdit = (j) => {
    setEditing(j);
    setForm({ ...j, deadline: j.deadline?.substring(0, 10) || "" });
    setShowModal(true);
  };
    ...
    <div className="premium-dashboard">
      <div className="dashboard-header">
        <div>
          ...
          <p className="dashboard-subtitle">Create and manage job listings</p>
        </div>
        <button
          onClick={() => {
            setEditing(null);
              ...
              <tbody>
                {jobs.map((j) => (
                  <tr key={j.id}>
                    ...
                    <td>
                      <span
                        className={`status-badge ${j.isActive ? "approved" : "rejected"}`}
                      >
