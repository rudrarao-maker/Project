...
export default function DigitalLockerPage() {
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this document from your locker?",
      )
    )
      return;
    try {
  ...
  return (
    <div className="premium-dashboard">
        ...
        >
          <div className="dashboard-panel p-6">
            <h2 className="panel-title mb-4">
              <Upload size={20} className="text-gov-blue" /> Add Document
            </h2>
            <form onSubmit={handleUpload} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                disabled={uploading}
                className="btn-solid-blue flex justify-center mt-2"
              >
                {uploading ? (
            ...
            <div className="p-6">
              {documents.length === 0 ? (
                <div className="flex-center flex-col py-16 text-gray-500 border-2 border-dashed border-gray-200 rounded-xl">
                  <ShieldCheck
                    size={48}
                    className="mb-4 opacity-50 text-gray-400"
                  />
                  <p className="font-medium text-lg text-gray-600">
                    Your locker is empty
                  </p>
                  <p className="text-sm">
                    Upload documents to use them instantly in service
                    applications.
                  </p>
                </div>
              ) : (
                <div className="grid sm:grid-cols-2 gap-4">
                  {documents.map((doc) => (
                    <div
                      key={doc.id}
                      className="border border-gray-200 rounded-xl p-4 flex gap-4 hover:shadow-md transition-shadow bg-gray-50/50"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-2 mt-4">
                          <button
                            onClick={() =>
                              setPreviewDoc({ ...doc, documentName: doc.name })
                            }
                            className="text-xs btn-outline-glow py-1 px-3 w-full justify-center"
                          >
                            View
                          </button>
                          <button
                            onClick={() => handleDelete(doc.id)}
                            className="text-xs bg-white text-red-500 border border-red-200 hover:bg-red-50 py-1 px-3 rounded-md w-full justify-center transition-colors"
                          >
                            Delete
                          </button>
                        </div>
      ...
      <FilePreviewModal
        ...
        fileUrl={
          previewDoc ? `http://localhost:5000${previewDoc.filePath}` : ""
        }
