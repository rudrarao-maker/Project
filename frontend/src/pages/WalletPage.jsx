  ...
  return (
    <div className="premium-dashboard">
      ...
      >
        <div>
          ...
          <p className="dashboard-subtitle">
            Manage your funds and pay for applications instantly
          </p>
        </div>
      </motion.div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        >
          <div
            className="dashboard-panel bg-gradient-to-br from-blue-900 to-blue-700 text-white p-6 relative overflow-hidden"
            style={{ background: "linear-gradient(135deg, #00508a, #002244)" }}
          >
            <div className="absolute top-0 right-0 p-4 opacity-20">
              <Wallet size={100} />
            </div>
            <h2 className="text-sm font-medium opacity-80 mb-2 uppercase tracking-wider">
              Available Balance
            </h2>
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  disabled={isAdding}
                  className="bg-white text-blue-900 px-4 rounded-lg font-bold hover:bg-gray-100 transition-colors"
                >
                  {isAdding ? (
                    <div className="spinner border-blue-900 w-5 h-5"></div>
                  ) : (
                    <Plus size={20} />
                  )}
                </button>
        ...
        <motion.div
          className="md:col-span-2 dashboard-panel"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
        >
          <div className="panel-header">
            <h2 className="panel-title">
              <Clock size={20} className="text-gov-blue" /> Recent Transactions
            </h2>
          </div>
                    ...
                    <tr key={tx.id}>
                      <td>
                        <div className="flex items-center gap-2">
                          <div
                            className={`p-1.5 rounded-full ${tx.type === "deposit" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
                          >
                            {tx.type === "deposit" ? (
                      ...
                      <td>
                        <span
                          className={`status-badge ${tx.status === "completed" ? "approved" : "pending"}`}
                        >
                          {tx.status}
                        </span>
                      </td>
