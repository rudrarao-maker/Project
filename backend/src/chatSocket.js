...
const initSocket = (server) => {
  ...
  io.on('connection', async (socket) => {
    ...
    if (socket.user.role === 'user') {
      ...
      io.to('admins').emit('user_connected', { userId: socket.user.id, sessionId: activeSession.sessionId });
    } else {
    ...
    socket.on('send_message', async (data) => {
      ...
      try {
        const sessionId = socket.user.role === 'user' ? activeSession.id : data.sessionId;
        const roomName = socket.user.role === 'user' ? activeSession.sessionId : data.sessionCode;
        
        const senderType = socket.user.role === 'user' ? 'user' : 'admin';
        
        const message = await saveMessage(sessionId, senderType, socket.user.id, data.content);
        ...
        if (senderType === 'user') {
           io.to('admins').emit('new_admin_message', { ...message, sessionCode: roomName });
        }
      } catch (error) {
