...
const authenticate = async (req, res, next) => {
  try {
    ...
    if (decoded.role && ['super_admin', 'admin', 'officer', 'reviewer'].includes(decoded.role)) {
      const admin = await prisma.admin.findUnique({
        where: { id: decoded.id },
        select: { id: true, adminId: true, name: true, email: true, role: true, status: true, department: true },
      });
      ...
      const user = await prisma.user.findUnique({
        where: { id: decoded.id },
        select: { id: true, userId: true, name: true, email: true, status: true, mobile: true },
      });
      if (!user || user.status === 'suspended') {
        return ApiResponse.error(res, 'Account is suspended or not found.', 401);
      }
